...
#include "logistic_regression.hpp"        // logistic regression for pp and non-pp settings
...
int main(){
  ...
  cout<<"\t [1] Binary MNIST"<<endl;
  cout<<"\t [2] Sanity Check"<<endl;
  int selection = 0;
  cout<<"Enter selection: ";
  cin>>selection;

  MatrixXd X,Y,w,X_testdata,Y_testdata;
  MatrixXd X_test, Y_test;
  ...
  if (selection == 1){ // loading Binary MNIST dataset
    ...
    MatrixXd X1_test(N_test, d); // 10000, 784
    MatrixXd Y1_test(N_test, 1); // 10000, 1
  ...
  MatrixXd ideal_w = idealLogisticRegression(X,Y,w);
