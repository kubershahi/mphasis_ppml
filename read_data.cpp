...
void read_insurance_data(string inputfile, vector<vector<double> > &X, vector<double> &Y){
  ...
  vector<double> temp;              // declaring a temp vector to hold content of a row

  double age, sex, bmi, children, smoker, charges;  // declaring the six features of the dataset
  ...
  if(fin.is_open()){               // if the dataset file is open  
    ...
    while(getline(fin,line)){      // getting the content of a line in variable line
      ...
      if (temp_sex == "male"){       // if the sex is male
        sex = 0.00; // male             // setting sex to 0 
      }
      else {
        sex = 1.00; // female           // else 1
      }
      ...
      if (temp_smoker == "yes"){     // if a smoker
        smoker = 1.00; //yes            // setting smoke to 1
      }
      else {
        smoker = 0.00;   //no           // else to a 1
      }
